...
from holdStrategy.helper import Holding, build_hold_strategy_prompt
...
def main(
    ...
    """Generate one hold-strategy report for one portfolio holding."""
    try:
        ...
        output_path = output_dir / f"{holding.ticker}.md"
    ...
    print(f"Hold strategy saved to {output_path}")
    return 0


if __name__ == "__main__":
    print(
        "Run hold strategies through run_hold_strategy.py so portfolio and "
        "window inputs are selected consistently.",
        file=sys.stderr,
    )
    raise SystemExit(2)
