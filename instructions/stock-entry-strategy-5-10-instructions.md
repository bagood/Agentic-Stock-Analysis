...
Convert a completed stock-analysis report into an executable, risk-controlled entry plan for the next **5–10 trading sessions**. Select **at least two distinct strategies** that offer the best supported trade-off between low downside risk and high attainable profit.
...
Create a comparison table containing trigger quality, downside per share/percent, Target 1 and Target 2 reward-to-risk, probability/expected-value evidence when usable, false-trigger risk, and horizon fit.
...
Include a compact daily monitoring checklist for closing price versus trigger/stop/targets, volume participation, money/foreign flow when supplied, momentum/trend confirmation, catalyst status, and sessions remaining.

## Required Output Structure

1. **Strategy timestamp, source cut-offs, and 5–10-session window**
...
10. **Daily monitoring checklist and end-of-window rule**
11. **Final decision**

Each strategy card must contain: setup, current state, entry trigger/zone, confirmations, cancellation condition, stop/invalidation, Target 1, Target 2, reward-to-risk calculations, expected value when defensible, time stop, maximum holding point, sizing note, and execution risks.

End with exactly these bullets:

- **Best Risk-Adjusted Entry Strategy:** Rank 1, its current state, trigger, stop, primary target, and conservative reward-to-risk.
- **Second-Best Entry Strategy:** Rank 2, its current state, trigger, stop, primary target, and conservative reward-to-risk; or state that no second setup qualifies.
- **Signals to Monitor:** The smallest set of decisive signals that moves either strategy from Waiting → Armed → Triggered → Active, plus invalidation.
