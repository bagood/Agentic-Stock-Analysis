# Instructions for Generating a Stock Hold Strategy (5 Trading Sessions)

## Purpose

Convert a completed stock-analysis report into a disciplined decision for an **existing stock position** within the source report’s fixed window of **5 trading sessions**. Determine whether the position should be:
...
- **Trading window:** Exactly 5 trading sessions
...
- Inherit the report’s verified schedule of exactly **5 exchange trading dates**, exchange time zone, and closing deadline for session 5. Intraday segments count together as one session. The original schedule begins at the first regular opening after the analysis timestamp and excludes weekends and official closures.
...
1. Use the report’s analysis timestamp, news cut-off, technical-data cut-off, reference price, and exact 5-session management window when making the decision. Include preparation time, relevant source cut-offs and reference-price date in Current Hold Decision, and the original dated expiry in Risk and Profit Plan.
...
Do not calculate total position P/L without quantity. Per-share P/L is valid without quantity. With quantity, gross position P/L equals quantity × per-share P/L; label fees, taxes, dividends, and other excluded adjustments. Reconcile splits, rights issues, and price-adjustment basis before comparing acquisition price, reference price, stops, or targets; withhold affected calculations when reconciliation is unsupported. Do not use sunk cost or the desire to “get back to break-even” as evidence.
...
- Whether the position can reasonably reach its objective inside 5 sessions.
...
| **Profit objective reached** | Supplied price has reached the target; execution is unconfirmed | Exact target condition | Take planned profit; reassess remainder |
| **Time exit** | Short-window thesis did not resolve | No required follow-through by the stated session or end of session 5 | Close or reanalyze as a new trade |
...
- First profit-taking zone and optional second zone only when supported and attainable within the remaining window.
- Conditions for partial profit, full exit, or continued hold after Target 1.
- A no-follow-through checkpoint at source-report session **2 or 3**: select the supported session, date it, and define the observation and action. If it has already passed, assess it now rather than restarting the count.
- Mandatory end-of-window action no later than session 5: close, take profit, or perform a fresh analysis before continuing.
...
End-of-window expiry is a decision deadline, not permission to wait for the final closing print before arranging an exit. Define a feasible pre-close exit plan when liquidation is intended. Any continuation requires a fresh analysis before the original session-5 deadline; if that is unavailable, state that this plan supplies no basis for continued holding and identify any already authorized time-exit instruction. An expired report used after the fact must not be presented as a fresh automatic sell recommendation.

## Required Output Structure

Return **only** the decision summary below. The heading must be exactly **Hold Strategy**. Do not output an introduction, validation section, P/L section, thesis comparison, calculations section, state table, management plan, monitoring checklist, disclaimer, conclusion, or any other text before or after it.
...
- [ ] The horizon is exactly 5 trading sessions.
